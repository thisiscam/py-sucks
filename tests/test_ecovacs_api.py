...
def test_md5():
    assert EcoVacsAPI.md5("fnord") == "b15e400c8dbd6697f26385216d32a40f"


def test_encrypt():
    assert len(EcoVacsAPI.encrypt("fnord")) == 172
...
def test_main_api_setup():
    with requests_mock.mock() as m:
        ...
        # verify state
        assert api.uid == "2017102559f0ee63c588d"
        assert api.login_access_token == "7a375650b0b1efd780029284479c4e41"
        assert api.auth_code == "5c28dac1ff580210e11292df57e87bef"
        assert api.user_access_token == "jt5O7oDR3gPHdVKCeb8Czx8xw8mDXM6s"
...
def test_device_lookup():
    ...
    with requests_mock.mock() as m:
        ...
        r = m.post(
            compile("user.do"),
            text='{"todo": "result", "devices": [{"did": "%s", "class": "126", "nick": "bob"}], "result": "ok"}'
            % device_id,
        )
        d = api.devices()
        ...
        assert len(d) == 1
        vacuum = d[0]
        assert vacuum["did"] == device_id
        assert vacuum["class"] == "126"
