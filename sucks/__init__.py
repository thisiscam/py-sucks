...
FAN_SPEED_TO_ECOVACS = {FAN_SPEED_NORMAL: "standard", FAN_SPEED_HIGH: "strong"}

FAN_SPEED_FROM_ECOVACS = {"standard": FAN_SPEED_NORMAL, "strong": FAN_SPEED_HIGH}
...
class VacBot:
    def __init__(
        ...
        self.xmpp = EcoVacsXMPP(
            user, domain, resource, secret, continent, server_address
        )
    ...
    def connect_and_wait_until_ready(self):
        ...
        if self._monitor:
            ...
            self.send_ping()
            self.xmpp.schedule(
                "Components", 3600, lambda: self.refresh_components(), repeat=True
            )
    ...
    def _handle_clean_report(self, event):
        ...
        if self.fan_speed:
            ...
            _LOGGER.debug(
                "*** clean_status = " + self.clean_status + " fan_speed = None"
            )
...
class EcoVacsXMPP(ClientXMPP):
    ...
    def register_callback(self, kind, function):
        self.register_handler(
            Callback(
                kind,
                MatchXPath(
                    '{jabber:client}iq/{com:ctl}query/{com:ctl}ctl[@td="' + kind + '"]'
                ),
                function,
            )
        )
    ...
    def _wrap_command(self, ctl, recipient):
        q = self.make_iq_query(xmlns="com:ctl", ito=recipient, ifrom=self._my_address())
        q["type"] = "set"
        for child in q.xml:
        ...
        super().__init__(
            "Clean",
            {
                "clean": {
                    ...
                    "speed": FAN_SPEED_TO_ECOVACS[speed],
                }
            },
        )
...
class Charge(VacBotCommand):
    def __init__(self):
        super().__init__(
            "Charge", {"charge": {"type": CHARGE_MODE_TO_ECOVACS["return"]}}
        )
